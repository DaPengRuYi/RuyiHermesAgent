...
delegate_task(tasks=[
    ...
    {
        "goal": "研究实际的量子计算应用",
        "context": "关注：纠错突破、真实世界用例、关键公司",
        "toolsets": ["web"]
    }
])
```

三个同时运行。每个子代理独立搜索网络并返回摘要。然后父代理将它们综合成一个连贯的简报。

---

## 模式：代码审查

将安全审查委托给一个新上下文的子代理，让它无偏见地接近代码：
...
delegate_task(
    ...
    context="""项目在 /home/user/webapp。Python 3.11，Flask，PyJWT，bcrypt。
...
:::warning 上下文问题
子代理对你的对话**一无所知**。它们完全从零开始。如果你委托 "修复我们讨论的 bug"，子代理不知道你指的是哪个 bug。始终显式传递文件路径、错误消息、项目结构和约束。
:::
...
对每种方法：设置复杂度、查询能力、资源需求和维护开销。
比较它们并推荐一个。
```

每个子代理独立研究一个选项。因为它们是隔离的，没有交叉污染 — 每个评估独立成立。父代理获得所有三个摘要并进行比较。
...
delegate_task(tasks=[
    {
        ...
        "context": """项目在 /home/user/api-server。
    ...
    },
    {
        ...
        "context": """项目在 /home/user/api-server。
        文档在：docs/api/。格式：带代码示例的 Markdown。
        将所有响应示例从旧格式更新为新格式。
        在 docs/api/overview.md 中添加 'Response Format' 部分解释 schema。""",
        "toolsets": ["terminal", "file"]
    }
])
```
...
:::tip
每个子代理获得自己的终端会话。它们可以在同一项目目录上工作而不会互相干扰 — 只要它们编辑不同的文件。如果两个子代理可能修改同一个文件，在并行工作完成后自己处理该文件。
:::
...
delegate_task(
    ...
    context="""/tmp/ai-funding-data.json 中的原始数据包含关于 2026 年 Q1
    AI 融资、收购和 IPO 的搜索结果和提取的网页。
    撰写结构化市场报告：关键交易、趋势、 notable 玩家和展望。
    关注 1 亿美元以上的交易。""",
    toolsets=["terminal", "file"]
)
```

这通常是最高效的模式：`execute_code` 便宜地处理 10+ 个顺序工具调用，然后子代理用干净的上下文做单个昂贵的推理任务。
...
限制工具集使子代理保持专注并防止意外副作用（如研究子代理运行 shell 命令）。

---

## 约束

- **默认 3 个并行任务**：批次默认为 3 个并发子代理（可通过 `config.yaml` 中的 `delegation.max_concurrent_children` 配置，无硬上限，仅下限为 1）
- **嵌套委托是可选加入的**：叶子子代理（默认）不能调用 `delegate_task`、`clarify`、`memory`、`send_message` 或 `execute_code`。编排器子代理（`role="orchestrator"`）保留 `delegate_task` 用于进一步委托，但仅当 `delegation.max_spawn_depth` 提高到默认的 1 以上时（支持 1-3）；其他四个仍然被阻止。通过 `delegation.orchestrator_enabled: false` 全局禁用。
...
| `max_spawn_depth` | 1 | 1-3 | 委托层级可以生成多少层 |

示例：运行 30 个并行工作者和嵌套子代理：
...
- **独立终端** — 每个子代理获得自己的终端会话，具有独立的工作目录和状态
- **无对话历史** — 子代理只看到父代理调用 `delegate_task` 时传递的 `goal` 和 `context`
...
**目标要具体。** "修复 bug" 太模糊。"修复 api/handlers.py 第 47 行的 TypeError，其中 process_request() 从 parse_body() 接收到 None" 给子代理足够的信息来工作。

**包含文件路径。** 子代理不知道你的项目结构。始终包含相关文件的绝对路径、项目根目录和测试命令。

**使用委托进行上下文隔离。** 有时你需要一个新视角。委托迫使你清晰地表达问题，子代理在没有你对话中积累的假设的情况下接近它。

**检查结果。** 子代理摘要只是摘要。如果子代理说 "修复了 bug 并且测试通过"，通过自己运行测试或阅读 diff 来验证。

---

*有关完整的委托参考 — 所有参数、ACP 集成和高级配置 — 请参见[子代理委托](/docs/user-guide/features/delegation)。*
