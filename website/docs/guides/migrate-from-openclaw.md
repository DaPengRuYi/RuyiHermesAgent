...
sidebar_position: 10
title: "从 OpenClaw 迁移"
description: "将你的 OpenClaw / Clawdbot 设置迁移到 Hermes Agent 的完整指南 — 迁移内容、配置映射方式和迁移后检查项"
---

# 从 OpenClaw 迁移

`hermes claw migrate` 将你的 OpenClaw（或旧版 Clawdbot/Moldbot）设置导入 Hermes。本指南涵盖具体迁移内容、配置键映射和迁移后验证项。

## 快速开始

```bash
# 预览然后迁移（始终先显示完整预览，然后要求确认）
hermes claw migrate
...
迁移在做任何更改前始终显示将导入内容的完整预览。查看列表，然后确认继续。
...
|------|----------------|-------------------|-------|
| 个性 | `workspace/SOUL.md` | `~/.hermes/SOUL.md` | 直接复制 |
...
工作区文件也会在 `workspace.default/` 和 `workspace-main/` 作为回退路径检查（OpenClaw 在最近版本中将 `workspace/` 重命名为 `workspace-main/`，并使用 `workspace-{agentId}` 用于多代理设置）。
...
| 自定义提供者 | `models.providers.*` | `config.yaml` → `custom_providers` | 映射 `baseUrl`、`apiType`/`api` — 处理短格式（"openai"、"anthropic"）和连字符格式（"openai-completions"、"anthropic-messages"、"google-generative-ai"）值 |
| 提供者 API 密钥 | `models.providers.*.apiKey` | `~/.hermes/.env` | 需要 `--migrate-secrets`。参见下面的 [API 密钥解析](#api-密钥解析)。 |
...
注意：OpenClaw 还有 `session.resetTriggers`（简单的字符串数组如 `["daily", "idle"]`）。如果结构化的 `session.reset` 不存在，迁移会回退到从 `resetTriggers` 推断。
...
|----------|---------------------|----------------------|-------|
| Telegram | `channels.telegram.botToken` 或 `.accounts.default.botToken` | `TELEGRAM_BOT_TOKEN` | 令牌可以是字符串或 [SecretRef](#secretref-处理)。支持扁平和 accounts 布局。 |
...
2. **环境文件** — `~/.openclaw/.env`（键如 `OPENROUTER_API_KEY`、`ANTHROPIC_API_KEY` 等）
3. **配置环境子对象** — `openclaw.json` → `"env"` 或 `"env"."vars"`（某些设置将密钥存储在这里而不是单独的 `.env` 文件）
4. **认证配置文件** — `~/.openclaw/agents/main/agent/auth-profiles.json`（每代理凭证）

配置值优先。每个后续来源填补任何剩余的缺口。
...
迁移解析所有三种格式。对于环境模板和 `source: "env"` 的 SecretRef 对象，它在 `~/.openclaw/.env` 和 `openclaw.json` 环境子对象中查找值。`source: "file"` 或 `source: "exec"` 的 SecretRef 对象无法自动解析 — 迁移会警告这些，这些值必须通过 `hermes config set` 手动添加到 Hermes。

## 迁移后

1. **检查迁移报告** — 完成时打印已迁移、已跳过和冲突项目的计数。
...
5. **测试消息** — 如果你迁移了平台令牌，重启网关：`systemctl --user restart hermes-gateway`

6. **检查会话策略** — 验证 `hermes config get session_reset` 符合你的预期。

7. **重新配对 WhatsApp** — WhatsApp 使用二维码配对（Baileys），不是令牌迁移。运行 `hermes whatsapp` 进行配对。
...
根据你的 OpenClaw 版本，密钥可能存储在多个地方：`openclaw.json` 中 `models.providers.*.apiKey` 下的内联值、`~/.openclaw/.env` 中、`openclaw.json` 的 `"env"` 子对象中、或 `agents/main/agent/auth-profiles.json` 中。迁移检查所有四个。如果密钥使用 `source: "file"` 或 `source: "exec"` SecretRef，它们无法自动解析 — 通过 `hermes config set` 添加。

### 迁移后技能未出现

导入的技能位于 `~/.hermes/skills/openclaw-imports/`。启动新会话使其生效，或运行 `/skills` 验证它们已加载。

### TTS 语音未迁移

OpenClaw 将 TTS 设置存储在两个地方：`messages.tts.providers.*` 和顶级 `talk` 配置。迁移检查两者。如果你的语音 ID 是通过 OpenClaw UI 设置的（存储在不同路径），你可能需要手动设置：`hermes config set tts.elevenlabs.voice_id YOUR_VOICE_ID`。
